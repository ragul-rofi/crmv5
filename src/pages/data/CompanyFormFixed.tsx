...
const baseSchema = z.object({
  ...
  employee_count: z.string().optional().nullable(), // Student Batch as string
...
const CompanyForm = ({ company, existingContacts = [], onSave, onCancel, isSaving }: CompanyFormProps) => {
  const { userProfile } = useUser();

  const [contactPersons, setContactPersons] = useState<ContactPerson[]>(() => {
    // If we have existing contacts, use them; otherwise, start with one empty contact
    ...
    return [{ id: crypto.randomUUID(), name: "", mobile: "", email: "" }];
  });
  ...
  const form = useForm<z.infer<typeof baseSchema>>({
    ...
    defaultValues: {
      ...
      employee_count: company?.employee_count || null, // Keep as string
  ...
  // Update contact persons when existingContacts changes
  useEffect(() => {
    if (existingContacts && existingContacts.length > 0) {
      setContactPersons(existingContacts.map(contact => ({
        id: contact.id || crypto.randomUUID(),
        name: contact.name || "",
        mobile: contact.phone || "",
        email: contact.email || "",
      ...
      // Reset to one empty contact when adding a new company
      setContactPersons([{ id: crypto.randomUUID(), name: "", mobile: "", email: "" }]);
    }
  }, [existingContacts, company]);
  ...
  const onSubmit = (values: z.infer<typeof baseSchema>) => {
    const mappedValues: Omit<Company, "id" | "created_at"> = {
      ...
      employee_count: values.employee_count || null, // Keep as string
